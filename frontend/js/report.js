...
function setPreviewLoading(previewEl) {
  previewEl.innerHTML = `
    <p class="text-muted">Loading cycle summary…</p>
  `;
}

function setPreviewError(previewEl, message) {
  previewEl.innerHTML = `
    <p class="text-muted">${message || "We couldn't load your cycle summary right now."}</p>
    <button id="retry-report-preview" class="btn btn-outline" type="button" style="margin-top:0.5rem;">
      Try again
    </button>
  `;
  const retryBtn = document.getElementById("retry-report-preview");
  if (retryBtn) retryBtn.onclick = () => loadPreview();
}
...
async function loadPreview() {
  ...
  try {
    ...
    console.error("[report] preview load failed:", err);
    setPreviewError(previewEl, "We couldn't load your report data right now. Please try again.");
    if (btn) btn.disabled = true;
  }
}
